...
export function getXPathForElement(el: Node, document: Document): string | void {
    ...
    while (element !== document.documentElement) {
        ...
        xpath = `*[name()='${element.nodeName}' and namespace-uri()='${element.namespaceURI === null ? '' : element.namespaceURI
            }'][${pos}]/${xpath}`;

        element = element.parentNode!;
    }

    xpath = `/*[name()='${document.documentElement.nodeName}' and namespace-uri()='${element.namespaceURI === null ? '' : element.namespaceURI
        }']/${xpath}`;
    xpath = xpath.replace(/\/$/, '');
